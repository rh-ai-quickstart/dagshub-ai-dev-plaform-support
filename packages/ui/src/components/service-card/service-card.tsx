...
const PACKAGE_INFO: Record<string, {
  ...
  Agent: {
    path: "packages/agent/",
    devUrl: "http://localhost:8000/agent",
    quickActions: [
      { label: "Agent Health", url: "http://localhost:8000/agent/health", external: true },
    ],
    commands: [
      { label: "Test", cmd: "pnpm test" },
      { label: "Lint", cmd: "pnpm lint" },
    ...
    gettingStarted: [
      "Add tools in \`agent/core/agent.py\`",
      "Configure providers in \`agent/core/config.py\`",
      "Add new agent routes in \`agent/routes/\`",
    ],
  },
  MCP: {
    path: "packages/mcp/",
    ...
    quickActions: [
      { label: "MCP Health", url: "http://localhost:3001/health", external: true },
    ],
    commands: [
      { label: "Dev", cmd: "pnpm dev" },
      { label: "Test", cmd: "pnpm test" },
    ...
    gettingStarted: [
      "Add tools via \`@mcp.tool()\` decorator in \`src/mcp.py\`",
      "Configure resources in \`src/resources.py\`",
      "Test with MCP Inspector at \`npx @modelcontextprotocol/inspector\`",
    ],
  },
  Chat: {
    path: "packages/chat/",
    ...
    gettingStarted: [
      "Run \`npx assistant-ui add thread\` to scaffold chat UI",
      "Import ChatPage component in your routes",
      "Configure AI provider in \`src/provider.tsx\`",
    ],
  },
  LlamaStack: {
    path: "packages/llamastack/",
    devUrl: "http://localhost:8321",
    quickActions: [
      { label: "LlamaStack Health", url: "http://localhost:8321/health", external: true },
    ],
    commands: [],
    gettingStarted: [
      "Deploy via Helm with \`make deploy\`",
      "Configure models in \`values.yaml\`",
      "Access API at \`http://localhost:8321\`",
    ],
  },
};
